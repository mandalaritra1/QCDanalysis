...
#         sel.add("npv", events.PV.npvsGood>0)
        ...
        if self.do_gen:
            ...
            #####################################
            sel.add("oneGenJet", 
                  ak.sum( (events.GenJet.pt > 136.) & (np.abs(events.GenJet.eta) < 2.5), axis=1 ) >= 1
            )
            events.GenJet = events.GenJet[(events.GenJet.pt > 136.) & (np.abs(events.GenJet.eta) < 2.5)]
        ...
        ### Reco jet selection
        #####################################
        recojets = events.Jet[(events.Jet.pt > 170.) & (np.abs(events.Jet.eta) < 2.5)  ] # &  get_dR( z_reco, events.Jet )>0.8
        sel.add("oneRecoJet", 
